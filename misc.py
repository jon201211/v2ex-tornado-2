...
class WorldClockHandler(BaseHandler):
    def get(self):
        site = GetSite()
        ...
        template_values['now'] = datetime.datetime.now(pytz.timezone('Asia/Shanghai'))
        path = os.path.join(os.path.dirname(__file__), 'tpl', 'desktop')
        t=self.get_template(path,'time.html')
        self.finish(t.render(template_values))

class MD5Handler(BaseHandler):
    def get(self, source):
        i = str(self.request.arguments['input'][0].strip())
        if (i):
...
class BFBCSPokeHandler(BaseHandler):
